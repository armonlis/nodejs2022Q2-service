...
export class AuthService {
  ...
  async login(data: LoginDTO) {
    ...
    const jwt = this.jwtService.sign(payload, {secret: SECRET_KEY, expiresIn: USE_BEFORE});
    const refresh_jwt = this.jwtService.sign(payload, { secret: REFRESH_SECRET_KEY, expiresIn: REFRESH_EXP });
    user.refresh_token = await bcrypt.hash(refresh_jwt, SALTROUND);
    await this.usersStorage.save(user);
    return { accessToken: jwt, access_exp: USE_BEFORE, refreshToken: refresh_jwt, refresh_exp: REFRESH_EXP };
  };

  async refresh(key: string) {
    const data = this.jwtService.decode(key) as JwtPayload;
    if (!data || data.exp * 1000 < Date.now() || !data.id) { return }
    const user = await this.usersStorage.findOneBy({ id: data.id });
    if (!user || user.login !== data.login) { return }
    const isTheSameKey = await bcrypt.compare(key, user.refresh_token);
    if (!isTheSameKey) { return }
    const payload = { login: user.login, id: user.id };
    const accessToken = this.jwtService.sign(payload, {secret: SECRET_KEY, expiresIn: USE_BEFORE});
    const refreshToken = this.jwtService.sign(payload, { secret: REFRESH_SECRET_KEY, expiresIn: REFRESH_EXP });
    user.refresh_token = await bcrypt.hash(refreshToken, SALTROUND);
    await this.usersStorage.save(user);
    return { accessToken, access_exp: USE_BEFORE, refreshToken, refresh_exp: REFRESH_EXP };
  }
