...
export class FavouritesService {
  ...
  private readonly favourites: IFavourites = {
    artists: [],
    tracks: [],
    albums: [],
  };
  ...
  async get() {
    const result = await this.favouritesStorage.findOneBy({ userId: this.userId });
    return result
  }

  async addArtist(id: string) {
    const favourites = await this.favouritesStorage.findOneBy({ userId: this.userId });
