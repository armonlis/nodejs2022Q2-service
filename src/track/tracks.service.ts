...
constructor (@Inject(forwardRef(() => ArtistsService)) private readonly artists: ArtistsService, 
  ...
  @Inject(forwardRef(() => FavouritesService)) private readonly favourites: FavouritesService) {};
...
get(id: string) { return this.tracks.find(track => track.id === id) };
...
setToNull(id: string, property: "album" | "artist" = "artist") {
  ...
  while (index !== -1) {
    ...
    if (index !== -1) { this.tracks[index][`${property}Id`] = null }
  };
};
